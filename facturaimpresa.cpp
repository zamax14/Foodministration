...
void facturaImpresa::insertarFactura(const Factura f)
{
    ...
    ui->ivaLB->setText("Iva: " + QString::number(f.getIva(),'g',2));
    ui->subtotalLB->setText("Subtotal: " + QString::number(f.getSubtotal(),'g',2));
    ui->totalLB->setText("Total: " + QString::number(f.getTotal(),'g',2));

    for (int i(0); i<f.Pedidos.size(); ++i) {
        QString formatoPedido = f.Pedidos.at(i).getNombre() + " - " + QString::number(f.Pedidos.at(i).getCantidad()) + " - $" + QString::number(f.Pedidos.at(i).getPrecio(),'g',2) + " - $" + QString::number(f.Pedidos.at(i).getCantidad()*f.Pedidos.at(i).getPrecio(),'g',2) + "\n";
